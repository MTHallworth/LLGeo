...
Now that we have adjusted the twilights - we have new information in out `twl` file. 

```{r}
str(twl)
```

In order to take advantage of the edits - you can subset the data using when `Deleted` = FALSE 
...
1.  `zenith` = the zenith angle to be used. This can take a single value (no change in zenith throughout the year) or a vector of `nrow(twl)` if you want to use different zenith angels.    
