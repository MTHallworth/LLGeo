...
Notice the difference in the `Light` measurement between the two types of tags. Migrate technology tags output light data in lux, and the values range from 0 to around 70,000 for full sunlight. Most other tags (including those from British Antarctic Survey or BioTrack) just have a somewhat arbitatry light intensity scale from 0 to around 64. 

When working with Migrate technology tags, it is sometimes useful to log-transform the light values to reduce the range of values.
...
All three of the analysis packages described here (GeoLight, SGAT, and FLightR) require as one of their inputs a dataframe containing the times of sunrise and sunset (henceforth twilights) for the duration of the study period. The twilight times are estimated based on a light-level threshold, which is the light value that seperates day from night - values above the threshold indicate the sun has risen and values below the threshold value indicate the sun has set. There are a few options for how to generate the twilight data. `twilightCalc` is one function that allows transitions to be defined and is part of the `GeoLight` package. Another function is called `preprocessLight`. Both require a threshold value be specified to define the transition events. 
...
`preprocessLight` is an interactive function for editing light data and deriving twilights. Note: if you are working on a Mac you must install Quartz first (https://www.xquartz.org) and then set gr.Device to "x11" in the function. If you are working with a virtual machine, the function may not work at all. Instructions of how to complete the interactive process can be found by running the following code. 
...
After you have defined the twilight periods this is what the first few lines of the `twl` object should look like:

```{r echo=FALSE}
head(twl)
```

If the `twl` object is empty check the following things: 1) If you used `locator()` function make sure you clicked on a date within the time frame that you may have subsetted above. 2) Double check that the light levels fall below the threshold level. 
...
This function provides a dataframe with both the edited twlight times (in column "Twilight") and the original twilight times (in column "Twilight0"). Now that we have adjusted the twilights - we have new information in out `twl` file. 
...
```{r echo = FALSE}
## Plot the edited data as a lightImage
lightImage(d.lig, offset = 18, zlim = c(0, 12), dt = 120)
tsimagePoints(twl$Twilight, offset = 18, pch = 16, cex = 0.5,
              col = ifelse(twl$Rise, "dodgerblue", "firebrick"))
```

In order to take advantage of the edits - you can subset the data using when `Deleted` == FALSE this removes any of the twilights which were deleted.

```{r}
twl <- subset(twl, Deleted == FALSE)
```

Many tags record the maximum light value measured within a given time interval, in our example data set it records the maximum light level every two mins. You can double check the time interval by looking at the `.lig` file. Because the maximum light level is stamped with a time that corresponds to the end of the sampling period, we may want to to adjust the sunset times to the beginning of the sampling period, when the tag was most likely to encounter the highest light level. Note that this adjustment is not necessary if you plan to use FLightR.
...
Now let's make a light image and use abline() to draw some orange lines that enclose the time period when the geolocator was on the bird.
```{r}
...
<img src="http://www.audubon.org/sites/default/files/styles/nas_bird_teaser_illustration/public/2972_Sibl_9780307957900_art_r1.jpg?itok=47NJq3By" width=200 height=200 />
...
Calibration
--------
Calibration serves two purposes in light leve geolocation. First it generates a sun-elevation angle (with respect to the horizon) or zenith angle (with respect to directly overhead), and second it allows us to quantify the error distribution around our estimates of sunrise and sunset. Calibration requires light level data taken at a known location so that we can compare the light level data to calculated sun positions. With this information we can first determine the sun angle associated with our threshold light value, and then use the threshold to generate estimates of sunrise and sunset that are compared with known (calculated) twilight time to give us an error distribution.
...
Capture location 
---------------
To perform calibration we must have data from a known location (usually the tag deployment or animal recapture site). It is best if the tag is on the animal during the calibration period. Let's create an object to store the capture location. 
...
Now that we have the capture location stored we need to make a subset of the twilight object (`twl`) we created earlier to include only dates when the animal was at the capture location. You can do this several ways. First, you can use personal observations (i.e., field work ended 1 August and the bird was still on territory) or the biology of the bird (i.e., stationary period ends when nestlings hatch around June 1). If you know an appropriate stationary period, then you can simply subset the data as follows:

```{r}
  cal <- subset(twl, twl$Twilight < as.POSIXct("2011-07-06", "GMT"))
```

This should give you all the twlights before July 6.

The alternative is to use the the light-level information direclty off the geolocator to determine when the animal was at the capture location. In order to use the second approach we need to know what the actual twilight events are and how they match up to the observed twilights derived from the geolocator data. 
...
You can get a quick and easy sun angle with the getElevation package in GeoLight. But first you have to get the calibration data into the special "tFirst/tSecond" format that GeoLight sometimes uses.
...
Determining the sun elevation angle--here called the Zenith angle--for SGAT. A Zenith angle is the angle of the sun with respect to a verticle line. If the sun is directly overhea, the angle is zero. If the sun is on the horizon, the angle is about 90 degrees. For an SGAT analysis it is often best to use a zenith angle from the high end of the distrubution of zenith angles derived from the calibration data. In the example below we take the 95th quantile. Remember that a larger zenith angle means that the sun is lower in the sky.
...
Here we assign the zenith that we will use in the analyses. If using a simple threshold approach (`GeoLight`) the median zenith Angle is probably good enough. 
...
Inital Path
-----------
Now that we have the error distribution around known sunrise and sunset times as well as the zenith angle, we can look at the location data. As a reminder, we are looking here at a subset o the data which includes twilights that 1) occurred after the first calibrartion date (presumably the deployment date) and 2) were not deleted according to the output of twilightEdit().  
...
Geolight has simple functions for generating coordinates from twlight data [coord()] and for plotting location data [tripMap()]. But first you have to get the data into the tFirst-tSecond format.
...
```{r eval = FALSE}
tripMap(crds = InitialPath$x, equinox = TRUE, xlim = c(-90,-70), ylim = c(10,50), legend = TRUE)
```
...
Creating a path with SGAT
-------------------------

The SGAT package contains functions that let you apply a movement model, landcover masking, and estimates of light detection error to generate a Markov Chain Monte Carlo algorithm that generates many likely movement paths from an individual data set. To run this sort of analysis we have to provide several parameters based on what we know about our data and the animal we are studying. We also have to provide an initial estimate of the movement path, which in this example will be the simple threshold path we generated above.
...
1. Define a movement model (usually a distribution of flight speeds or daily flight distances wherein long flights are rare and short flights are common.)
1. Establish the error distribution for twilight detection based on your calibration data
1. (Optional) Establish a mask or grid that delineates location that are extremely unlikely to be part of the movement path (e.g. small landbirds will not reside on large water bodies).
...
1. Run the model several hundred times to create the final path

First, we rename the initial path from our results above (the mapped route), and then find the midpoints between each pair of consecutive locations.

### Initialize the SGAT Estella model

```{r echo=FALSE}
  # Take the location estimates created above
  x0 <- InitialPath$x
  
  # the will need the mid-points between locations - generate those here
  z0 <- trackMidpts(x0)
```

Next, we create an object called fixedx that specifies the known locations for some or all of the calibration data and categorizes those locations as fixed so that the model does not try to estimate them.
...
Now let's generate some parameters for a basic movement model. We need to provide a mean and standard deviation for a gamma distribution of flight speeds that get applied to each day of the analysis period. We typically want short (near zero) distance flights to be common and long distance flights to be relatively rare. So both mean and distribution should be small.

```{r echo=FALSE}  
  # Flight model
beta <- c(0.7, 0.08) #mean and sd 
## plot the gamma distribution with the establishe parameters
...
We include a prior distribution so only locations on land within the Americas are used. We first create a function to covert the `wrld_smpl` shapefile to a binary surface.
...
Set the x and y limits for the raster surface. It's important that you set these limits to include all longitude and latitude values where the individual may occur throughout the year. If your xlim and ylim don't include all areas you won't get results for that portion of the range.
...
Here we define the distribution mask to only include stationary locations that occur on land. This makes sense for something like a Tree Swallow but may not for other species - **always** consider the ecology of the species while conducting the analysis. 
...
The log prior function produces a probability value for locations on land and locations at sea. If you feed the function a set of coordinates that is at sea the prior probilbity is -10
...
We then fit the model using the `estelleMetropolis` sampler. Here you can set the number of iterations (`iters`), the thinning rate (`thin`) and the number of chains to run (`chains`).
...
We can then summarize the data using the `slices` function. Here we "slice" the intermediate locations by day. You can set `breaks` to day, month, etc. 
...
Next if we are interested in certain days or a period of interest you may want to pull out locations that fall within a certain time period. Here we extract the dates from the MCMC process. 

```{r}
  DATES <- S$mcmc[[1]]$time[ which( S$mcmc[[1]]$rise==TRUE) ]
```
  
We can create objects that correspond with certain dates of interest. These objects are the rows that correpsond with the dates of interest. 
...
We can then use the `sliceInterval` function to extract and summarize the locations between the dates of interest. For example, if we were interested in time of year when an individual is on the breeding grounds - we may specify the dates from the `ReleaseDay` until the end of the breeding season - say `Aug01`. We may also be interested in the stationary non-breeding period - say between `Jan01` and `Mar01`. ***Note*** these dates were chosen haphazardly for this analysis. When conducting your own analysis make sure to consider the ecology of your species. 
...
<img src="http://www.audubon.org/sites/default/files/styles/nas_bird_teaser_illustration/public/1405_Sibl_9780307957900_art_r1.jpg?itok=juwNVz1Q" width=300 height=300 />
...
Again, it's good practice to save the twilight file so that you have a record of the twilights that were edited / deleted and make your research reproducible. 
