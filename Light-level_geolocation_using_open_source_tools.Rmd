...
Determining the sun elevation angle--here called the Zenith angle--for SGAT. For an SGAT analysis it is often best to use a zenith angle from the high end of the distrubution of zenith angles derived from the calibration data. In the example below we take the 95th quantile. Note that a larger zenith angle means that the sun is lower in the sky.
...
Geolight has simple functions for generating coordinates from twlight data [coord()] and for plotting location data [tripMap()]. But first you have to get the data into tFirst, tSecond format.
...
You can use tripmap to show the path, as we did above. But here is some alternative code for
making figures to show the inital path. 
...
Here is the function to create a distribution mask. 
